...
import SampleBackendData from './sampleBackendData';
...
export default function Dashboard() {
    ...
    useEffect(() => {
        if (process.env.DEPLOYMENY_TYPE === "docker-compose") {
            console.log('Docker compose deployment detected ...')
            fetch('http://0.0.0.0:9000/testAPI')
                .then(response => response.json())
                .then(data => console.log(data.rows));
        } else {
            console.log('Development environment without docker-compose detected ...');
            console.log(rows);
        }
